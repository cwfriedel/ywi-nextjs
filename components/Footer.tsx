...
export default function Footer(){
  const footerLinks = navLinks.filter(n => n.href !== '/' && n.href !== '/contact')
  return (
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10 grid sm:grid-cols-3 gap-8">
        <div>
          ...
          <p className="text-sm text-stone mt-2">
            Promoting the sustainable use of natural resources and protecting
            long-term biological diversity within the Yuba River watershed.
          </p>
        </div>
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {footerLinks.map(n => (
              <li key={n.href}><Link className="link" href={n.href}>{n.label}</Link></li>
            ))}
          </ul>
        </div>
        <div>
          ...
          <p className="text-sm text-stone">
            Your gift sustains forest health projects, ecological restoration, and educational events.
          </p>
